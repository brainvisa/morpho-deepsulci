...
class ErrorComputation(Process):
    ...
    t1mri: field(type_=File, doc="MRI scan")
    true_graph: field(type_=File, doc="corresponding graph manually labeled")
    labeled_graphs: field(
        type_=File, doc="corresponding set of graphs automatically labeled"
    )
    sulci_side_list: field(
        type_=list[str],
        doc="list of sulci (e.g. S.C._right) considered to compute the"
        " error rates. It is not supposed to contain the labels"
        ' "unknown", "ventricle_left" and "ventricle_right".',
    )
    error_file: field(type_=File, write=True, doc="file (.csv) storing the error rates for each labeled graph")
    ...
    def execution(self, context):
        ...
        for gfile in self.labeled_graphs:
            ...
            y_pred = [
                vor[int(round(p[0])), int(round(p[1])), int(round(p[2]))][0]
                for p in bck
            ]
            ...
            for ss in self.sulci_side_list:
                ...
                re.ix[gfile, "TP_" + str(ss)] = (
                    float(len(names_ss[names_ss == ss])) * vvol
                )
                re.ix[gfile, "FP_" + str(ss)] = (
                    float(len(labels_ss[labels_ss != ss])) * vvol
                )
                re.ix[gfile, "FN_" + str(ss)] = (
                    float(len(names_ss[names_ss != ss])) * vvol
                )
            ...
            for ss in self.sulci_side_list:
                ...
                if FP + FN + 2 * VP != 0:
                    re.ix[gfile, "ESI_" + str(ss)] = (
                        s / sum_s * (FP + FN) / (FP + FN + 2 * VP)
                    )
                    re.ix[gfile, "Elocal_" + str(ss)] = (
                        s / sum_s * (FP + FN) / (FP + FN + VP)
                    )
                else:
        ...
        for ss in self.sulci_side_list:
            print(
                "%s Elocal mean: %.3f, max: %.3f"
                % (ss, re["Elocal_" + str(ss)].mean(), re["Elocal_" + str(ss)].max())
            )
